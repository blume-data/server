...
const EntriesTableComponent = (props: EntriesTableType) => {
  ...
  useEffect(() => {
    const newRows = response.map((i: any) => {
      const assetTypeRows =
        rules && rules.filter((rule) => rule.type === MEDIA_FIELD_TYPE);
      const jsonTypeRows =
        rules && rules.filter((rule) => rule.type === JSON_FIELD_TYPE);
      const richTypeRows =
        rules && rules.filter((rule) => rule.type === LONG_STRING_FIELD_TYPE);
      ...
      if (assetTypeRows && assetTypeRows.length) {
        assetTypeRows.forEach((assetTypeRow) => {
          ...
          if (
            assetTypeRow.assetsType === SINGLE_ASSETS_TYPE &&
            i[assetTypeRow.name]
          ) {
            component = (
              <AvatarCommon
                alt={assetTypeRow.name}
                src={i[assetTypeRow.name].thumbnailUrl}
              />
            );
  ...
  async function getItems(fieldTitleParam?: string) {
    if (
      ...
      const getOnly = [
        ...
        ENTRY_UPDATED_BY,
        STATUS,
        fieldTitleParam ? fieldTitleParam : fieldTitle,
        "id",
      ];
  ...
  function updateColumns() {
    const newColumns: any[] = [{ name: "Id", value: "id" }];
  ...
  async function fetchModelDataAndRules() {
    if (GetCollectionNamesUrl && applicationName) {
      const response = await getModelDataAndRules({
        ...
        language,
        env,
        getOnly: "rules,displayName,id,titleField",
      });
  ...
  return (
    <Grid className={"entries-table-container-wrapper"}>
