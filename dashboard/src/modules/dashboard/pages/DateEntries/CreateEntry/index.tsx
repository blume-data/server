                ...
                switch (ruleExist.type) {
                  case JSON_FIELD_TYPE: {
                    if (typeof response.data[0][prop] !== "string") {
                      newResponse[prop] = JSON.stringify(
                  ...
                  case DATE_AND_TIME_FIElD_TYPE: {
                    ...
                    const timeStamp = DateTime.fromISO(response.data[0][prop], {
                      zone: `${response.data[0][`${prop}-timezone`]}`,
                    }).toJSDate();
      ...
      values.forEach((valueItem: { name: string; value: string }) => {
        ...
        if (rules && rules.length) {
          ...
          if (exist) {
            // check reference
            if (
              exist.type === REFERENCE_FIELD_TYPE &&
              exist[REFERENCE_MODEL_TYPE] === ONE_TO_MANY_RELATION
            ) {
            ...
            // check assets
            if (
              exist.type === MEDIA_FIELD_TYPE &&
              exist.assetsType === MULTIPLE_ASSETS_TYPE
            ) {
