...
import {
  ...
  ACTION_FETCH_DATA_ADDRESS_ROUTES,
  AppThunk,
...
export const fetchAssetsRouteAddress =
  (): AppThunk => async (dispatch: ThunkDispatch<any, any, AnyAction>) => {
    dispatch({
...
export const fetchDataRouteAddresses =
  (): AppThunk => async (dispatch: ThunkDispatch<any, any, AnyAction>) => {
    dispatch({
...
export const fetchApplicationNames =
  ...
  async (dispatch: ThunkDispatch<any, any, AnyAction>, getState: any) => {
    ...
    if (state && state.routeAddress) {
      const ApplicationNameUrl =
        state.routeAddress.routes.data?.ApplicationNameUrl;
      const clientUserName = getItemFromLocalStorage(CLIENT_USER_NAME);
      const url = ApplicationNameUrl.replace(
        `:${CLIENT_USER_NAME}`,
