...
export const Form = (props: FormType) => {
  ...
  function changeValue(event: any, field: string, action: string) {
    ...
    function setHelperText(
      fieldItem: ConfigField,
      formStateItem: FormState,
      value: string,
      whenTouched = false
    ) {
      ...
      if (max !== undefined || min !== undefined) {
        if (max !== undefined && max) {
          if (
            (inputType === BIG_TEXT || inputType === TEXT) &&
            type === "text"
          ) {
            if (
              value.length > max &&
      ...
      return !value && fieldItem.required
        ? setErrorMessage(formStateItem.label)
        : "";
    }
  ...
  function renderFields(field: ConfigField, index: number, groupId?: string) {
    ...
    const classNames = `${className} form-field-container-wrapper ${
      error ? "red-border" : ""
    }`;
      ...
      return (
        <Suspense key={index} fallback={<div />}>
          <DateField
            ...
            onChange={(value: any) => {
              changeValue({ target: { value } }, label, SET_VALUE_ACTION);
            }}
            onBlur={onBlur}
            label={label}
            id={id}
    ...
    if (inputType === FORMATTED_TEXT) {
      return (
        <HtmlEditor
          ...
          label={label}
          setValue={(str: string) =>
            setHtmlEditorValue(str, label, SET_VALUE_ACTION)
          }
          value={value}
        />
      );
    ...
    formState.forEach((item) => {
      if (errors && errors.length) {
        const field = fields.find(
          (fieldItem) => fieldItem.label === item.label
        );
        if (field) {
          const error = errors.find(
            (errorItem) => errorItem[FIELD] === field.name
          );
          if (error) {
            values.push({
              ...item,
          ...
          {filteredGroups && filteredGroups.length
            ? filteredGroups.map((groupName, index) => {
                const exist = fields.filter(
                  (ranjod) => ranjod.groupName === groupName
                );
        ...
        <Grid item>
          ...
          >
            <Grid container justify="space-between" alignItems="center">
              ...
              {loading ? (
                <Grid style={{ flex: 1 }} className="center">
                  <CircularProgress
                    style={{
                      color: "white",
                      height: "20px",
